...
# ================= DASHBOARD =================
    ...
    # ================= 3 PANEL LAYOUT =================
    left_col, mid_col, map_col = st.columns([2,1,1])
    ...
    # -------- EMERGENCY MAP --------
    with map_col:
        st.subheader("🚑 Emergency Maps")

        location = st.text_input("Enter Location", "Pune")
        ...
        if st.button("Find Nearby"):
            ...
            st.markdown(
                f"""
                <iframe src="{map_url}" width="100%" height="400"></iframe>
                """,
    ...
    while st.session_state.running:
        ...
        try:
            ...
            people_metric.metric("People Count", person_count)

            # -------- CROWD GRAPH --------
            st.session_state.crowd_data.append(person_count)

            if len(st.session_state.crowd_data) > 30:
